...
We will be modeling on 2 different dataframes. One with AcidIndex and LabelAppeal as numeric variables and the second one with factorizrd AcidIndex and LabelAppeal
...
But there is no Multicollinearity noticed for any of the variables in wine.trn1.numeric.omit.na dataframe.
There for we will keep all the variables for modelling.
...
pois.manual0 <- step(glm(TARGET ~ VolatileAcidity + Chlorides + FreeSulfurDioxide + TotalSulfurDioxide + 
    Density + Sulphates + LabelAppeal + AcidIndex + STARS, data=wine.training, family=poisson()), trace = FALSE)
...
nbm.manual0 <- step(glm.nb(TARGET ~ VolatileAcidity + Chlorides + FreeSulfurDioxide + TotalSulfurDioxide + 
     Sulphates + LabelAppeal + AcidIndex + STARS, data=wine.training0),trace = FALSE)
