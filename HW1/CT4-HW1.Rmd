...
#Full Model
model.manualElimination <- lm(formula = TARGET_WINS ~ . -INDEX -TEAM_BASERUN_CS -TEAM_BATTING_HBP -TEAM_BATTING_SO,data =moneyballTraining)
...
The adjusted r-squared value in the stepwise backward model is `r ar4`
...
```{r, warning=FALSE, message=FALSE}
model.vif <- lm(TARGET_WINS ~ .-INDEX -TEAM_BATTING_HR -TEAM_PITCHING_HR -TEAM_BATTING_BB -TEAM_PITCHING_BB -TEAM_BATTING_3B -TEAM_BATTING_2B -TEAM_BATTING_SO -TEAM_BATTING_HBP -TEAM_PITCHING_H -TEAM_PITCHING_SO, data=moneyballTraining)
...
##4.1. RMSE - Root Mean Sqared Error (verification with test data)
```{r}
#Lets take our model, and apply it on the test dataset.
predicted.wins <- predict(model.vif, newdata = moneyballTest)
...
plot(model.vif, which=3)


#For normality of residuals
plot(model.vif, which=2) #if most of the resids are on the straight line we are good to go.

#Lets check normality via ggplot for specific data element
# say, TEAM_BATTING_HR
d = data.frame(x=model.vif$residuals, y = moneyballTraining$TEAM_BATTING_HR)
...
eval.wins <- predict(model.vif, newdata = moneyballEvaluation)
