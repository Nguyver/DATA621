...
Nitrogen oxides concentration is the least deviation, so this variable may be dropped from the model.

All the models provided us with the below model formula:

*target ~ nox+rad+tax*

Let us try to apply the performance measurs on this.
...
*Apply the performance measures on the selected model [ target ~ nox+rad+tax ]* 
...
performance.results <- data.frame( model = character(),  Sn = numeric(), Sp = numeric, Accuracy = numeric())
```
...
performance.results <- rbind(performance.results, c("GLM - Combined [Forwared, Backward, Manual]",sensitivity, specificity,  accuracy))
...
```{r}
colnames(performance.results) <- c("Method", "Sn", "Sp", "Accuracy")
kable(performance.results)
```
