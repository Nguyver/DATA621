...
par(mfrow=c(2, 2))
graphics::plot(model.backward.step.log, main="Stepwise Backward Regression")
```
...
par(mfrow=c(2, 2))
graphics::plot(model.forward.step.log, main="Stepwise Forward Regression")
```
...
par(mfrow=c(2, 2))
graphics::plot(model.backward.step.mlr, main="Stepwise Backward Regression")
```
...
model.forward.step.mlr <- step(nothing.mod.lnr,direction = 'forward', trace=1)

summary(model.forward.step.mlr)
formula(model.forward.step.mlr)

par(mfrow=c(2, 2))
graphics::plot(model.forward.step.mlr, main="Stepwise Forward Regression")
```
...
#### Stepwise Backward Regression Performance:
...
From the above, both the models result in poor performance in predicting the target variable of amount from the given dataset. So, predicting the target crash amount would be difficult with our above assumptions. However, for our evaluation lets consider the Step wise Backward regression method.
...
Though it was easier to predict the *target flag* - which indicates if someone gets into a crash or not, it was not easier to predict the *TARGET AMOUNT* which indicates how much amount will be claimed. So, the target claim amount from a crash seemes to be more random.


```{r}
#Predict Target Flag using Logistic Regression
insurance.target.flag.prd <- predict(model.backward.step.log,newdata=subset(insurance.evl,select=c(HOMEKIDS , PARENT1 , HOME_VAL , TRAVTIME , BLUEBOOK , 
    TIF , OLDCLAIM , CLM_FREQ , REVOKED , MVR_PTS , CARUSE_COMMERCIAL , 
    EDU_BACH_MAST_PHD , JOB_WHITECOLLAR , KIDSDRIV_2 , MARRIED , 
    CAR_TYPE_vAN , CAR_TYPE_SUV , CAR_TYPE_SPORTS , CAR_TYPE_PICKUP , 
    CAR_TYPE_PANTRUCK , URBAN)),type='response')
