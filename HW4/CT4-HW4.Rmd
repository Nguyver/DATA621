...
### Dummy Variables
 ...
 These variables are identified as categorical, and would need to be converted into Dummy Variables for Model buidling
...
```{r}
gg.age <- ggplot(data = insurance.trn) + geom_histogram(aes(x=AGE)) +  theme(axis.text=element_text(size=8), axis.title=element_text(size=8))

gg.bluebook <- ggplot(data = insurance.trn) + geom_histogram(aes(x=BLUEBOOK)) +  theme(axis.text=element_text(size=8), axis.title=element_text(size=8))

gg.carage <- ggplot(data = insurance.trn) + geom_histogram(aes(x=CAR_AGE)) +  theme(axis.text=element_text(size=8), axis.title=element_text(size=8))

#Total Clamins (Past 5 Years) , Lets take average and draw the histogram.
insurance.trn$OLDCLAIM.AVG <- insurance.trn$OLDCLAIM/5
gg.oldclaimavg <- ggplot(data = insurance.trn) + geom_histogram(aes(x=log(OLDCLAIM.AVG)), binwidth = 0.5) +  theme(axis.text=element_text(size=8), axis.title=element_text(size=8))
insurance.trn$OLDCLAIM.AVG <- NULL
#People who have been customers for a long time are usually more safe.
gg.tif <- ggplot(data = insurance.trn) + geom_histogram(aes(x=TIF)) +  theme(axis.text=element_text(size=8), axis.title=element_text(size=8))

#TRAVTIME - Long drives to work usually suggest greater risk
gg.traveltm <- ggplot(data = insurance.trn) + geom_histogram(aes(x=TRAVTIME)) +  theme(axis.text=element_text(size=8), axis.title=element_text(size=8))

#YOJ - People who stay at a job for a long time are usually more safe
gg.yoj <- ggplot(data = insurance.trn) + geom_histogram(aes(x=YOJ)) +  theme(axis.text=element_text(size=8), axis.title=element_text(size=8))

grid.arrange(gg.age, gg.bluebook, gg.carage, gg.oldclaimavg,gg.tif,gg.traveltm,gg.yoj, ncol=2)
```

From the above histogrames, the 'BLUE BOOK', 'TIF' distributions appears to be positively skewed. We modified the OLDCLAIM variable be the average per year and applied log to make the data distribution nearly normal. We still could see a slight bi-model in it, but we will see how it affect our models.
...
From the above table, we do see multi-colinearity (with VIF > 10) among the predictors - INCOME_LOG,HOME_VAL_LOG variables. 
...
From the above residual plots, shows that the residuals are not fully linear and the residual variance is not constant. The Normal Q-Q graph indicates that the most of the residuals are on the straight line.However, the Residual Vs Leverage plot has the redline  alligned with gray dotted line, which indicates that the assumption of standardized residuals centered around zero is some what true here.

The significant variables include: 

*INCOME, TRAVTIME, BLUEBOOK, TIF,CLM_FREQ,REVOKED,MVR_PTS,CARUSE_COMMERCIAL* 
*EDU_BACH_MAST_PHD,JOB_WHITECOLLAR,KIDSDRIV_2 and ,MARRIED,CAR_TYPE,URBAN*

For a unit increase in REVOKED there is a chance of 89% of crash.

And also commercial vehicles tend to be resulting in increased chance of a crash ( approx. 70% per unit increase in commericial car use).
...
The resuls are pretty much similar to what we have seen in the backward regression above. 

The residual plots, shows that the residuals are not fully linear and the residual variance is not constant. The Normal Q-Q graph indicates that the most of the residuals are on the straight line.However, the Residual Vs Leverage plot has the redline  alligned with gray dotted line, which indicates that the assumption of standardized residuals centered around zero is some what true here.

For a unit increase in REVOKED there is a chance of 89% of crash.

And also commercial vehicles tend to be resulting in incrased chances of a crash ( approx. 70% per unit increase in commericial car use).
...
```{r}
model.manual.fit.log <- stats::glm(TARGET_FLAG ~ +AGE +CARUSE_COMMERCIAL +CLM_FREQ +HOME_VAL_LOG +JOB_WHITECOLLAR +KIDSDRIV_2 +KIDSDRIV_4 +MARRIED +MVR_PTS +OLDCLAIM_YRLY_AVG +REVOKED +MALE +TIF +TRAVTIME +YOJ -TARGET_AMT , family=binomial(), data =insurance.training)
...
The resuls are some what similar to the above 2, however, the normality in residuals is better in stepwise models than our manual model.

In our manual model, we noticed that REVOKED increases the crash chance by almost 100%. And HOME_VAL decreases the decreases the chances of crash significantly. And the families with 1 or 2 teenage kids shows higher chances of crash.
And the commercial car usage results in 57% crash increase.
...
In the residuals Vs Fitted graph, the red line is about flat, which indicates the linearity in residuals is good. However the residual variance is  NOT constant. The Normal Q-Q graph indicates some of the residuals are on the straight line. However, the Residual Vs Leverage plot has the redline slightly alligned with gray dotted line, this indicates that the assumption of standardized residuals centered around zero is true here, though there are few outliers.

The significant predictors in the above are: 

*INCOME,BLUEBOOK,TRAVTIME,TIF,REVOKED,MVR_PTS,CARUSE_COMMERCIAL*
*KIDSDRIV_2,MARRIED,CAR_TYPE_SUV,CAR_TYPE_SPORTS and URBAN*

Per unit increase in the URBAN , the target amount increases by 1340. And SPORTS CARs causes in increase of 1199 target amount.
Similary we can see that the *MARRIED, WHITE COLLAR, TIF, HOME_VAL_LOG* are actually causes decrease in the CLAIM AMOUNT.


#### Stepwise Forward Linear Regression:
```{r}
nothing.mod.lnr <- lm(TARGET_AMT ~ 1, data=insurance.training)
model.forward.step.mlr <- step(nothing.mod.lnr, scope=list(lower=formula(nothing.mod.lnr),upper=formula(fit.mlr)), direction = 'forward', trace=FALSE)
...
The resuls are VERY similar to the Stepwise backward linear regression model.

In the residuals Vs Fitted graph, the red line is about flat, which indicates the linearity in residuals is good. However the residual variance is  NOT constant. The Normal Q-Q graph indicates some of the residuals are on the straight line. However, the Residual Vs Leverage plot has the redline slightly alligned with gray dotted line, this indicates that the assumption of standardized residuals centered around zero is true here, though there are few outliers.

The significant predictors in the above are: 

*MVR_PTS,PARENT1,TRAVTIME,TIF,REVOKED,MVR_PTS,INCOME,CARUSE_COMMERCIAL* 
*,MARRIED,CAR_TYPE_SUV,CAR_TYPE_SPORTS,KIDSDRIV_2,TRAVTIME,TIF and URBAN*

Per unit increase in the URBAN , the target amount increases by 1395 And SPORTS CARs causes in increase of 1056 target amount.
Similary we can see that the *MARRIED, TIF* are actually causes decrease in the CLAIM AMOUNT.
...
```{r}
model.manual.fit.mlr <- stats::lm(TARGET_AMT ~ +AGE +CARUSE_COMMERCIAL +CLM_FREQ +HOME_VAL_LOG +JOB_WHITECOLLAR +KIDSDRIV_2 +KIDSDRIV_4 +MARRIED +MVR_PTS +OLDCLAIM_YRLY_AVG +REVOKED +MALE +TIF +TRAVTIME +YOJ -TARGET_FLAG , data =insurance.training)
...
sensitivity <- round(sensitivity(factor(insurance.test$predicted),insurance.test$TARGET_FLAG, positive="1"), 4)
...
performance.results1 <- rbind(performance.results1, c("Step wise Backward",sensitivity, specificity,  accuracy, auc))
```
...
graphics::plot(roc, legacy.axes = TRUE, col="blue", lwd=3)
auc <- round(auc(factor(predicted)~as.numeric(TARGET_FLAG),insurance.test),4)
performance.results1 <- rbind(performance.results1, c("Step wise Forward",sensitivity, specificity,  accuracy, auc))
```
...
#Adjusted R squared
ar2 <-  round(summary(model.backward.step.mlr)$adj.r.squared , 4)

#Root mean sqaure error:
rmse= round(sqrt(mean((predict(model.backward.step.mlr,insurance.test)-insurance.test$TARGET_AMT)**2)),4)
...
```
R squared -proportion of variation in the dependent (response) variable
...
```{r}
#Adjusted R squared
ar2 <-  round(summary(model.forward.step.mlr)$adj.r.squared,4)

#Root mean sqaure error:
rmse= round(sqrt(mean((predict(model.forward.step.mlr,insurance.test)-insurance.test$TARGET_AMT)**2)),4)
...
## Evaluation:
 ...
 Lets apply the final models on the evaluation dataset. We will first tidy the evaluation dataset and then predit the *TARGET_FLAG* and *TARGET_AMT*
...
insurance.evl$INCOME_LOG<- ifelse(insurance.evl$INCOME_LOG ==0, median(insurance.evl$INCOME_LOG), insurance.evl$INCOME_LOG)
...
insurance.evl$HOME_VAL_LOG<- ifelse(insurance.evl$HOME_VAL_LOG ==0, median(insurance.evl$HOME_VAL_LOG), insurance.evl$HOME_VAL_LOG)

insurance.evl = insurance.evl %>% mutate(CARUSE_COMMERCIAL=as.numeric(CAR_USE=="Commercial"))  %>% select(-CAR_USE)

insurance.evl = insurance.evl %>% mutate(EDU_BACH_MAST_PHD=as.numeric(EDUCATION %in% c("PhD", "Masters", "Bachelors")))  %>% select(-EDUCATION)

insurance.evl = insurance.evl %>% mutate(JOB_WHITECOLLAR=as.numeric(JOB %in% c("Manager", "Lawyer", "Doctor", "Professional"))) %>% select(-JOB)


insurance.evl = insurance.evl %>% mutate(KIDSDRIV_2=as.numeric(KIDSDRIV %in% c(1,2)),
                                         KIDSDRIV_4=as.numeric(KIDSDRIV %in% c(2,4))) %>% 
                                  select(-KIDSDRIV)
...
insurance.evl = insurance.evl %>% mutate(RED_SPORTS_CAR=as.numeric(RED_CAR  == "yes" & CAR_TYPE == 'Sports Car')) %>% 
                                  select(-RED_CAR)
...
insurance.evl = insurance.evl %>% mutate(URBAN=as.numeric(URBANICITY  == "Highly Urban/ Urban")) %>% 
                                  select(-URBANICITY)
...
insurance.target.flag.prd <- predict(model.backward.step.log, newdata=subset(insurance.evl,select=c(HOMEKIDS ,YOJ, PARENT1 , TRAVTIME , BLUEBOOK , TIF , CLM_FREQ , REVOKED , MVR_PTS ,OLDCLAIM_YRLY_AVG, HOME_VAL_LOG, CARUSE_COMMERCIAL , 
