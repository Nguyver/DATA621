...
model.step.log = step(fit.log , trace = FALSE) 

summary(model.step.log) 
formula(model.step.log)

par(mfrow=c(2, 2))
graphics::plot(model.step.log, main="Stepwise Regression")
```

```{r}

fullmod.lgr <- stats::glm(TARGET_FLAG ~ .-TARGET_AMT, family=binomial(), data=insurance.training)
insurance.trn.backmod.lgr <- step(fullmod.lgr, trace=0)
formula(insurance.trn.backmod.lgr)
par(mfrow=c(2, 2))
graphics::plot(insurance.trn.backmod.lgr)

# 2.
nothing.mod.lgr <- glm(TARGET_FLAG ~ 1,family=binomial, data=na.omit(insurance.training))
insurance.trn.forwardmod.lgr = step(nothing.mod.lgr,
    scope=list(lower=formula(nothing.mod.lgr),upper=formula(fullmod.lgr)), direction="forward", trace = FALSE)
formula(insurance.trn.forwardmod.lgr)
...
fit.mlr <-  stats::lm(formula = TARGET_AMT ~. -TARGET_FLAG , data = insurance.training)
model.step.mlr <- step(fit.mlr, trace=FALSE)
summary(model.step.mlr)

par(mfrow=c(2, 2))
graphics::plot(model.step.mlr, main="Stepwise Regression")
```

```{r}
insurance.training$INDEX <- NULL
```


```{r}
fullmod.lnr <-  stats::lm(TARGET_AMT ~ . -TARGET_FLAG, data=insurance.training)
insurance.trn.backmod.lnr <- step(fullmod.lnr, trace=0)

par(mfrow=c(2, 2))
graphics::plot(insurance.trn.backmod.lnr)


nothing.mod.lnr <- lm(TARGET_AMT ~ 1, data=insurance.training)
insurance.trn.forwardmod.lnr <- step(nothing.mod.lnr,
...
```{r}
insurance.test$score=predict(model.step.log,insurance.test,type="response")
...
performance.results <- rbind(performance.results, c("Step wise",sensitivity, specificity,  accuracy, auc))
```
...
#Adjusted R squared
(ar2 <-  summary(model.step.mlr)$adj.r.squared)


#Root mean sqaure error:
(rmse=sqrt(mean((predict(model.step.mlr,insurance.test)-insurance.test$TARGET_AMT)**2)))

plot(insurance.test$TARGET_AMT,predict(model.step.mlr,insurance.test))
...
#insurance.evl$EDUCATION <- sapply(insurance.trn$EDUCATION, cleanUpPrefix)
...
#insurance.prd <- predict(model.step.log,newdata=subset(insurance.evl,select=c(2,4,6,8,9,10,11,13,14,15,16,17,18,19,21,22,23,24,26)),type='response')
